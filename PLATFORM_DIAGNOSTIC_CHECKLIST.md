...
- [ ] Add landing page with feature overview before auto-guest mode
- [ ] Add onboarding wizard/modal on first visit explaining portfolio tools
- [ ] Add financial glossary tooltips throughout UI (Sharpe ratio, HHI, beta, alpha, basis points, etc.)
- [ ] Add contextual help icons (?) on complex UI elements with explanations
- [ ] Add keyboard shortcuts help overlay (press `?` to toggle)

### Core Missing Features
- [ ] Add CSV/PDF export for portfolios and analytics data
- [ ] Add portfolio comparison view — side-by-side metrics for 2+ saved portfolios
...
- [ ] Show "Expected Return" source label — clarify if historical, projected, or model-based
...
- [ ] Improve mobile table layouts — stack rows vertically or add collapsible detail rows
...
- [ ] Add liquidity constraints (max % of 20-day volume per position)
- [ ] Calibrate stress test scenarios to historical events (2008, COVID, Taper Tantrum)
- [ ] Add predictive drift monitoring — forecast 30-day drift and rebalance urgency
...
- [ ] Add audit logging table and log all sensitive operations (password changes, admin actions, portfolio saves)
- [ ] Add pagination to `/api/stocks/all` endpoint
- [ ] Add caching to persona config endpoint (`GET /api/personas`)
- [ ] Add automated PostgreSQL backup container in Docker Compose
- [ ] Add Caddy security headers, rate limiting, and request size limits
- [ ] Serve frontend with nginx instead of `npm install -g serve`
- [ ] Add missing test coverage: session expiry, token reuse, admin access control, CSRF validation, rate limit backoff
- [ ] Add constant-time session validation to prevent timing attacks
- [ ] Add progressive backoff on password reset token brute-force attempts
- [ ] Sanitize URL paths in logs — strip query parameters containing tokens

### Nice-to-Have Features
- [ ] Add price alerts / notifications system
- [ ] Add portfolio sharing (read-only link)
...
- [ ] Add ticker autocomplete/suggestions in Market Data search
- [ ] Add stock comparison — side-by-side view for 2+ tickers
- [ ] Add FAQ / help documentation section
...
|-------|-------|------|-----------|
| Phase 1 — Critical Fixes | 17 | 0 | 17 |
| Phase 2 — Financial Model | 14 | 0 | 14 |
| Phase 3 — UX & Features | 20 | 0 | 20 |
| Phase 4 — Professional | 23 | 0 | 23 |
| **Total** | **74** | **0** | **74** |
